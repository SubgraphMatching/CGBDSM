...
// share memory这块，再想想怎么优化，MAX_VCOUNT设置为16会导致share memory过多导致并发度较低，再想想再想想
            ...
            else
            {
                ...
                while (compact_count[warp_id] < WARP_SIZE && read_offset < read_end)
                {
                    ...
                    if (read_offset + lane_id < read_end)
                    {
                        ...
                        if ((C_VALID_BITS.bits_[current_qv][nbr / 32u] & (1u << (nbr % 32u))) == 0u)
                            nbr = UINT32_MAX;
                    }
                    __syncwarp();
...
__global__ void writeInitialResultsAllBit(
    ...
    uint32_t* vb_qv0 = vb_ptrs[ei * MAX_VCOUNT + qv0];
    uint32_t* vb_qv1 = vb_ptrs[ei * MAX_VCOUNT + qv1];
    ...
    {
        if ((vb_qv0[v / 32u] & (1u << (v % 32u))) == 0u) continue;
        uint32_t size = sizes[v], my_count = 0;
        for (uint32_t j = lane_id; j < size; j += WARP_SIZE) {
            uint32_t nbr = nbrs[v][j];
            if ((vb_qv1[nbr / 32u] & (1u << (nbr % 32u))) != 0u)
                my_count++;
        }
...
__global__ void extendBFSAllBit(
    ...
    __shared__ uint32_t compact_pos[NWARP_PER_BLOCK][WARP_SIZE * 2];
    __shared__ uint8_t compact_count[NWARP_PER_BLOCK];
    __shared__ uint32_t lb_cache[NWARP_PER_BLOCK][WARP_SIZE];
    ...
    const uint16_t ei_vb_base = ei * MAX_VCOUNT;
    if (lane_id < C_QV_COUNT - 2)
    {
        queue_pos[warp_id][lane_id] = 0u;
        queue_size[warp_id][lane_id] = (lane_id < start_depth - 2) ? 1u: 0u;
            ...
            else
            {
                ...
                while (compact_count[warp_id] < WARP_SIZE && read_offset < read_end)
                {
                    ...
                    if (read_offset + lane_id < read_end)
                    {
                        ...
                        if ((vb_ptrs[ei_vb_base + current_qv][nbr / 32u] & (1u << (nbr % 32u))) == 0u)
                            nbr = UINT32_MAX;
                    }
                    __syncwarp();
                ...
                if (write_res && *new_res_size >= h_max_new_res_size_) return;
                {
                    for (uint8_t off = C_GLOBAL_ORDER.bni_offs_[depth[warp_id]] + 1; off < C_GLOBAL_ORDER.bni_offs_[depth[warp_id] + 1]; off++)
                    {
                        ...
                        load_lb_cache(lb_cache[warp_id], d_all_local[ei].nbrs_[pre_pre_qe_idx][pre_pre_v], base_sz);

                        if (found)
                        {
                            uint32_t lb = lower_bound_2phase(d_all_local[ei].nbrs_[pre_pre_qe_idx][pre_pre_v], base_sz, temp_nbr, lb_cache[warp_id]);
