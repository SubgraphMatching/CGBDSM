...
void CaLiGHelper::ConvertGlobalIndex(uint32_t *cardinalities, float *degrees) {
    ...
    {
        thrust::host_vector<uint64_t> h_el = el_dev;
        uint32_t total = h_el.size();
        std::vector<uint32_t> counts_per_edge(num_edges, 0);
        for (uint32_t i = 0; i < total; i++) {
            uint32_t gamma_eidx = (uint32_t)(h_el[i] >> 59);
            counts_per_edge[gamma_eidx]++;
        }
        gpma_ = new CGCSM_GPMA();
        gpma_->Init(num_edges, DV_COUNT);
        gpma_->BulkLoad(thrust::raw_pointer_cast(el_dev.data()), total, counts_per_edge.data());
        printf("[CaLiGHelper] GPMA loaded with %u total edges\n", total);
    }
}
...
void CaLiGHelper::PrepareUpdateIndex(uint32_t batch) {
    ...
    tbb::parallel_for(0u, (uint32_t)updates.size(), [&](uint32_t i) {
        ...
        uint64_t data = (uint64_t)((uint64_t)gamma_eidx << 59 | (uint64_t)new_u << 32 | new_v);
        tmp_el[tid].push_back(data);
        local_flat[tid].push_back(EdgeItem{gamma_eidx, d.vi, d.vj});
    });
...
bool CaLiGHelper::TransferUpdateIndex(uint32_t batch) {
    ...
    if (el_update_dev.empty()) return false;
...
void CaLiGHelper::UpdateGlobalIndex(uint32_t batch, uint8_t cur_i) {
    ...
    {
        uint32_t total = local_stage.size();
        std::vector<int32_t> counts_per_edge(num_edges, 0);
        for (uint32_t i = 0; i < total; i++) {
            uint32_t gamma_eidx = (uint32_t)(local_stage[i] >> 59);
            counts_per_edge[gamma_eidx]++;
        }
        gpma_->BatchUpdate(local_stage.data(), total, nullptr, 0, counts_per_edge.data(), num_valid_vertices_buf_[batch]);
    }
}

void CaLiGHelper::PrintUpdateIndexSizes(uint32_t batch) {
}
